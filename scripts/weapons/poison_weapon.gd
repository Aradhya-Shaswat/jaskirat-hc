...
var poison_audio: AudioStreamPlayer3D
var is_firing := false
...
func _ready() -> void:
    ...
    poison_audio = AudioStreamPlayer3D.new()
    poison_audio.stream = load("res://sounds/Free Sounds Pack/Magical Interface 5-1.wav")
    add_child(poison_audio)
...
func fire(delta: float) -> void:
    ...
    # if poison_audio and not poison_audio.playing:
    #     poison_audio.play()
...
func stop_fire() -> void:
    ...
    if poison_audio and poison_audio.playing:
        poison_audio.stop()
